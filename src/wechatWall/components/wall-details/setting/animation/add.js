...
export default class EditComp extends React.Component {
	...
	handleSubmit() {
		...
		this.props.form.validateFields((errors, values) => {
			...
			if(!this.state.supportList.length) delete values.support_name
			this.state.supportList.forEach(item => {
				sponsor_name.push(item.support_name)
				values.sponsor_name = sponsor_name.join()
				sponsor_image.push(item.support_img)
				values.sponsor_image = sponsor_image.join()
			})
			
			this.props.actions.updateTemplateList('add', values).then(resolve => {
	...
	addSupport() {
		...
		const support_name = values.support_name
		const key = Math.random().toString(32).substr(2, 8)
		const support_img = this.state.support_img
		const supportList = this.state.supportList
		supportList.push({ support_name, support_img, key })
			...
			<div>
				<Form horizontal style={{ marginTop: 40 }}>
			        ...
			        <FormItem  {...formItemLayout} label="消息条目数：" hasFeedback>
	    	        	<Select  {...msgNumProps} size="large" placeholder="请选择消息条目数" >
	    	        		{
			        ...
			        <FormItem  {...formItemLayout} label="消息框透明度：" hasFeedback>
	    	        	<Slider {...getFieldProps('msg_frame_clarity')} style={{wdith:350}}/>
			        </FormItem>
			        ...
			        <FormItem
			          ...
			          hasFeedback
			          >
			          <Input {...getFieldProps('qr_code_desc')} type="textarea" rows="6"/>
			        </FormItem>
			        ...
			        <FormItem
			          ...
			          hasFeedback
			          >
			          <Input {...getFieldProps('screen_explain')} type="textarea" rows="6"/>
			        </FormItem>
