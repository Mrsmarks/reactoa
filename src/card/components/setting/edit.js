...
export default class MainModal extends React.Component{
	...
	componentWillReceiveProps(nextProps) {
		const option = nextProps.option.toJS()
		const info = nextProps.info.toJS()
		const id = this.context.location.query.id
		this.setState({ready: this.state.ready+1})
		if(info.id && info.id == id && this.state.ready == 1) {
			const index_name = option.index.find(item => item.id == info.index).name
			const list_name = option.list.find(item => item.id == info.list).name
			const greeting_name = option.greeting.find(item => item.id == info.greeting).name
			this.setState({
				index_id: info.index,
				list_id: info.list,
				greeting_id: info.greeting,
				index_name: index_name,
				list_name: list_name,
				greeting_name: greeting_name,
				cover_img: info.cover_image,
				water_img: info.watermark,
			})
		}
	}
	...
	handleSumbit() {
		...
		this.props.form.validateFields((err, values) => {
			...
			this.props.actions.updateSettingList(values, info.id).then(resolve => {
	...
	render() {
		return (
			<Spin spinning={this.props.singerLoading}>
				<div>
					{this.renderForm()}
					<Modal
						title="首页模板"
