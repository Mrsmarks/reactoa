...
export default class QRCodeComp extends React.Component {
	...
	handleSearch(nextPage = this.props.params.get('page'), pageSize = this.props.params.get('psize')) {
		const query = {
			psize: pageSize,
			page: nextPage,
			name: this.props.form.getFieldProps('searchName').value
		}
	...
	renderToolbar() {
		const nameProps = this.props.form.getFieldProps('searchName', {
	...
	renderTable() {
		...
		const columns = [{
			...
			key: 'qrcode',
			render(_, obj) {
				return <IconFont onClick={showQRCode(obj)} type="icon-eye" cursor title="查看"/>
			}
		}, {
			title: '位置信息',
	...
	renderModal() {
		...
		const channelProps = getFieldProps('channel', {
			rules: [
		...
		const typeProps = getFieldProps('action_name', {
			rules: [
		...
		const expireDateProps = getFieldProps('expire_date', {
			initialValue: format(data.expire_date*1000)
		})

		let expireProps
		if (!hasData) {
			expireProps = getFieldProps('expire_seconds', {
				rules: [
			...
			>
				<Form horizontal >
			        ...
			        <FormItem
						...
						hasFeedback
					>
						<Select {...typeProps} disabled={!!hasData}>
							{
			        ...
			        </FormItem>
			        {
			        	...
			        	<FormItem
							...
							required
						>
							<InputNumber {...expireProps} min={1} max={7}/>
			        ...
			        }
			        {
			         ...
			         <FormItem
						...
						hasFeedback
					 >
						<Input {...expireDateProps} disabled={!!hasData}/>							
			         </FormItem> : ''
			       }
