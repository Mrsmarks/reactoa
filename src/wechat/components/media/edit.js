...
export default class MediaEditCom extends React.Component {
	...
	renderForm() {
		...
		const mediaTypeProps = getFieldProps('media_type', {
			rules: [
		...
		const videoTitleProps = getFieldProps('title', {
			initialValue: editData.get('title')
		})

		const videoDescProps = getFieldProps('introduction', {
		...
		return(
			<Form horizontal >
		        ...
		        <FormItem  {...formItemLayout} label="有效期类型：">
	        		<Select {...typeProps} disabled={!!editData.size} placeholder="请选择类型" style={{ width: 150 }}>
    	        		{
	        	...
	        	<FormItem  {...formItemLayout} label="素材类型：">
    	        	<Select {...mediaTypeProps} disabled={!!editData.size} placeholder="请选择类型" style={{ width: 150 }}>
    	        		{
    	        			mediaType.map(item => 
    	        				<Option key={item.get('id')} value={item.get('id')+''}>{item.get('name')}</Option>
    	        			)
        		...
        		{{
        			...
        			'3': ([
						...
						<FormItem key={2} {...formItemLayout} label="视频标题：">
	        				<Input {...videoTitleProps} type="text"/>
						</FormItem>,
						<FormItem key={3} {...formItemLayout} label="视频描述：">
	        				<Input {...videoDescProps} type="textarea"/>
						</FormItem>
        			]),
        			'4': (
        				<FormItem {...formItemLayout} label="上传图片：">
