...
export default class ReplyComp extends React.Component {
	...
	handlePageChange(nextPage) {
		...
		this.props.actions.fetchReplyList({page: nextPage})
	}
	...
	renderToolbar() {
		...
		const keywordProps = getFieldProps('name', {
		...
		return (
			<div className="toolbar">
				<Form inline >
                    ...
                    <FormItem  label="匹配类型：">
                        <Select {...mstchingProps} size="large" placeholder="请选择匹配类型" style={{ width: 150 }}>
                            <Option key={'x'} value="-1">全部</Option>
                            {
					...
					<span style={{marginLeft:5}}> </span>
		        	<FormItem  label="关键字：">
	    	        	<Input {...keywordProps} size="large" placeholder="请输入关键字查询"/>
		        	</FormItem>
				...
				return (
					<div>
						<Auth type={["wechat-reply-check"]}>
							<a onClick={toChildRoute('/wechat/reply/edit', obj.id)} style={{paddingRight:5}}>详情</a>
						</Auth>
