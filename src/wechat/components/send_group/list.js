...
export default class SendGroupComp extends React.Component {
	...
	saveSendGroup() {
		...
		validateFields(['group_name'], (error, values) => {
			if (error) {
				return
			}
			let {
			...
			const postData = {
				...
				check_openid: this.state.groupSendPreviewValue,
				type: groupSendType,
	...
	renderToolbar() {
		...
		const nameProps = getFieldProps('searchName', {
			initialValue: params.name
		})
		const msgTypeProps = getFieldProps('msgtype', {
			initialValue: params.msgtype
		})
		const statusProps = getFieldProps('send_status', {
			initialValue: params.send_status
		})
		const typeProps = getFieldProps('searchType', {
			...
			<div className="toolbar">
				<Form inline>
					...
					<FormItem  label="群发内容类型：">
	    	        	<Select {...msgTypeProps} size="large" placeholder="请选择类型" style={{ width: 150 }}>
		        	...
		        	<FormItem  label="群发内容对象：">
	    	        	<Select {...typeProps} size="large" placeholder="请选择对象" style={{ width: 150 }}>
	    	        		<Option key={'x'} value="-1">全部</Option>
	    	        		{
	    	        			sendType.map(item =>
	    	        				<Option key={item.id} value={''+item.id}>{item.name}</Option>
		        	...
		        	<FormItem  label="发送状态：">
	    	        	<Select  {...statusProps} size="large" placeholder="请选择类型" style={{ width: 150 }}>
	    	        		<Option key={'x'} value="-1">全部</Option>
	    	        		{
	    	        			sendStatus.map(item => 
	    	        				<Option key={item.id} value={''+item.id}>{item.name}</Option>
	...
	renderEditModal() {
		...
		const nameProps = getFieldProps('group_name', {
			rules: [
		...
		const groupSendPreview = getFieldProps('groupSendPreview', {
			initialValue: safeString(editData.check_openid)
		})

		const groupSendType = getFieldProps('groupSendType', {
    		onChange: this.setModalSelectId.bind(this, 'sendTypeId'),
    		initialValue:  safeString(editData.type)
    	})

    	const wechatGroupId = getFieldProps('wechatGroupId', {
    		initialValue: editData.wechat_groupid ? +editData.wechat_groupid : undefined
    	})

    	const virtualGroupId = getFieldProps('virtualGroupId', {
    	...
    	// }
    	const userId = getFieldProps('userId', {
    		// initialValue: userIdInitialValue
    	})

    	const groupSendContentType = getFieldProps('groupSendContentType', {
			...
			>
				<Form horizontal >
			        ...
			        <FormItem 
			        	...
			        	label="群发类型："
			        >
	    	        	<Select style={selectStyle} {...groupSendType}>
	    	        		{
	    	        			sendType.map(item => 
	    	        				<Option key={item.id} value={item.id+''}>{item.name}</Option>
	    	        			)
		        	...
		        	{{
		        		'2': (
		        			<FormItem 
					        	...
					        	label="微信分组："
					        >
			    	        	<Select {...wechatGroupId} style={selectStyle}>
			    	        		{
		        	...
		        	<FormItem 
		        		...
		        		label="群发内容类型："
		        	>
	    	        	<Select style={selectStyle} {...groupSendContentType}>
	    	        		{
	    	        			massType.map(item => 
	    	        				<Option key={item.id} value={item.id+''}>{item.name}</Option>
	    	        			)
		        	...
		        	{{
		        		'mpnews': (
		        			<FormItem 
				        		...
				        		label="图文："
				        	>
			    	        	<Select {...imgTxtId} style={selectStyle}>
			    	        		{
			    	        			allTxt.map(item => 
		        		...
		        		'text': (
		        			<FormItem 
				        		...
				        		label="文本："
				        	>
			    	        	<Input {...txtId} type="textarea" rows="5"/>
				        	</FormItem>
		        		),
		        		'voice': (
		        			<FormItem 
				        		...
				        		label="语音："
				        	>
			    	        	<Select {...voiceId} style={selectStyle}>
			    	        		{
			    	        			allVoice.map(item => 
		        		...
		        		'image': (
		        			<FormItem 
				        		...
				        		label="图片："
				        	>
			    	        	<Select {...imgId} style={selectStyle}>
			    	        		{
			    	        			allImage.map(item => 
			    	        				<Option key={item.media_id} value={`${item.media_id}@${item.name}`}>{item.name}</Option>
			    	        			)
		        		...
		        		'mpvideo': (
		        			<FormItem 
				        		...
				        		label="视频："
				        	>
			    	        	<Select {...videoId} style={selectStyle}>
			    	        		{
			    	        			allVideo.map(item => 
			    	        				<Option key={item.media_id} value={`${item.media_id}@${item.name}`}>{item.name}</Option>
			    	        			)
