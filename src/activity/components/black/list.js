...
export default class GroupComp extends  React.Component {
	...
	handleSearch(e) {
    	...
    	this.props.form.validateFields((errors, values) => {
			...
			this.props.actions.fetchBlackList({page, type, aid, value_num})
    	})
	}
	...
	handlePageChange(nextPage, pageSize) {
		...
		this.props.actions.fetchBlackList(query)
	}

	handleAdd(info) {
		this.props.actions.addBlackList(info).then(resolve => {
	...
	handleUpdate(info, id) {
		this.props.actions.updateBlackList(info, id).then(resolve => {
			message.success(resolve.errormsg)
	...
	handleRemove(id) {
		this.props.actions.delBlackList(id).then(resolve => {
			message.success(resolve.errormsg)
		})
	}

	uploadFile(file) {
		this.props.actions.uploadExcelFile(file, '/activity/activity-black-list/import-data').then(resolve => {
			message.success(resolve.errormsg)
		})
	}
		...
		return (
			<div className="toolbar">
				<Form inline >
					...
					<span style={{marginLeft: 5}}></span>
					<Auth type={["activity-black-list-add"]}>
						<Button type="ghost" onClick={() => {window.location.href = `${this.props.backend_domain}/template/activity-black-list.xlsx`}}>
							<Icon type="download" />
							导入模板下载
						</Button>
					</Auth>
