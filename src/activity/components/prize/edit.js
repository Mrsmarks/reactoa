...
export default class EditComp extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if(info.id && info.id == id) {
			...
			for(var i = 1; i < 6; i++) {
				if(info[`screenshots${i}`]){
					let obj = {}
					obj.uid = Math.random().toString(36).substr(2, 7)
					obj.status = 'done'
					obj.url = assetsUrl + info[`screenshots${i}`]
					fileList.push(obj)
				}
			}
  	...
  	handleSubmit(e) {
    	...
    	this.props.form.validateFields((errors, values) => {
			...
			this.props.actions.updatePrizeList(values, info.id).then(resolve => {
	...
	render() {
		...
		const psidProps = getFieldProps('psid', {
			...
			initialValue: info.psid
		}) 

		const prizeRuleProps = getFieldProps('prize_rule', {
			initialValue: info.prize_rule
		})
		...
		const customProps = getFieldProps('custom_service', {
			...
			initialValue: info.custom_service
		})
				...
				<Spin spinning={!this.state.ready}>
					<Form horizontal style={{ marginTop: 40 }}>
				        ...
				        <FormItem  {...formItemLayout}>
				        	<Col offset="9">
				        	<Auth type={["activity-prize-update"]}>
